...
file_path_Team_data = os.path.join(bot_folder, "config", "player_observation_data.json")
file_path_Team_data = os.path.normpath(file_path_Team_data)
content = {}
file_path_Team_data = if_json_file_404(file_path_Team_data, content)
        ...
        if player_id == None:
            ...
            async def my_callback(interaction):
                ...
                if f"{select.values[0]}" == f"🆕Create a new team.🆕":
                    ...
                    JSOn_data = read_json_file(file_path_Team_data)
...
class Delt_player(commands.Cog):
    ...
    async def choise_player_to_delt(self, interaction: discord.Interaction):
        ...
        dict = read_json_file(file_path_Team_data)
    ...
    async def on_message(self, message):
        ...
        if message.channel.id != player_observation_channel_id:
            ...
            if message.content not in observation_commands:
                ...
                embed_attention = discord.Embed(title="Please Note: Observation Channel Usage",
                      description=description, colour=0x004000)
